...
class ViewController: UIViewController, UITextFieldDelegate{
    ...
    /*
        guard let gotoHome = self.storyboard?.instantiateViewController(identifier: "mainView", creator: nil) as? MainViewController else {
            return
        }
        self.navigationController?.pushViewController(gotoHome, animated: true)

        guard let gotoFriend = self.storyboard?.instantiateViewController(identifier: "FriendView", creator:nil) as? FriendViewController else {return}
        self.navigationController?.pushViewController(gotoFriend, animated: true)
*/
    }
    func layout(){
